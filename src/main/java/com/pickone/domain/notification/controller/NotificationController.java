...
@Tag(name = "Notification", description = "알림(Notification) 관련 API")
public class NotificationController {
  ...
  @GetMapping("/{userId}")
  public ResponseEntity<BaseResponse<List<NotificationDto>>> getUserNotifications(
  ...
  public ResponseEntity<BaseResponse<NotificationDto>> sendNotification(
    ...
    NotificationDto notification = commandService.sendNotification(
        request.userId(),
        request.message(),
        request.type()
    );
  ...
  public ResponseEntity<BaseResponse<Void>> markAsRead(
      @Parameter(description = "알림 ID") @PathVariable String notificationId) {
