...
class AuthServiceTest {
    ...
    void setUp() {
        ...
        when(termService.getAll()).thenReturn(List.of(
                new TermEntity(1L, "서비스 이용약관", "내용입니다.", "v1.0", true, LocalDateTime.now()),
                new TermEntity(2L, "마케팅 정보 수신", "동의하시면 이벤트 정보를 받아볼 수 있습니다.", "v1.0", false, LocalDateTime.now())
        ));
    }
    ...
    void signup_success() {
        SignupRequestDto request = new SignupRequestDto(
                "user@example.com", "password123", "닉네임",
        ...
        doNothing().when(emailVerificationService).sendVerificationEmail(any()); // ✅ 추가
    ...
    void change_password_same_as_old() {
        ...
        when(passwordEncoder.matches("old-password", "encoded-old")).thenReturn(true); // 동일하다고 가정
